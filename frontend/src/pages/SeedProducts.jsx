...
export default function SeedProducts() {
  ...
  async function handleSeedProducts() {
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        setMessage(`Adding products... ${i + 1}/${total}`);
      }

      setMessage(`✅ Successfully added ${total} products!`);
      setTimeout(() => {
        navigate('/');
      }, 2000);
    } catch (error) {
  ...
  return (
    <div className="page-container" style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center', padding: '3rem 1rem' }}>
      <h1 style={{ color: '#667eea', marginBottom: '1rem' }}>🛒 Add Sample Products</h1>
      <p style={{ color: '#666', marginBottom: '2rem', fontSize: '1.1rem' }}>
        Click the button below to add 13 hardware products to your store.
      </p>
      ...
      <button
        ...
        disabled={seeding}
        style={{
          background: seeding ? '#ccc' : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
          color: 'white',
          border: 'none',
          padding: '1rem 2.5rem',
          borderRadius: '8px',
      ...
      >
        {seeding ? '⏳ Adding Products...' : '✨ Add Products Now'}
      </button>
      ...
      {message && (
        <p style={{
          padding: '1rem',
          borderRadius: '8px',
          background: message.includes('✅') ? '#d1e7dd' : message.includes('❌') ? '#f8d7da' : '#cfe2ff',
          color: message.includes('✅') ? '#0f5132' : message.includes('❌') ? '#842029' : '#084298',
          fontWeight: '600'
        }}>
          {message}
        </p>
